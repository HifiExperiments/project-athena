    ...
    AbstractViewStateInterface::instance()->pushPostUpdateLambda(key, [weakSelf]() {
        ...
        for (int i = 0; i < (int) self->_modelMeshRenderItemIDs.size(); i++) {
            ...
            if (skinDeformerIndex != hfm::UNDEFINED_KEY) {
                ...
                transaction.updateItem<ModelMeshPartPayload>(itemID, [modelTransform, meshState, useDualQuaternionSkinning, invalidatePayloadShapeKey,
                                                                      primitiveMode, billboardMode, renderItemKeyGlobalFlags, cauterized, renderWithZones](ModelMeshPartPayload& data) {
                ...
                transaction.updateItem<ModelMeshPartPayload>(itemID, [modelTransform, invalidatePayloadShapeKey, primitiveMode, billboardMode,
                                                                      renderItemKeyGlobalFlags, cauterized, renderWithZones](ModelMeshPartPayload& data) {
                    data.updateTransformForUnskinnedMesh(modelTransform);
                    //data.updateTransformAndBound(modelTransform.worldTransform(shapeState._rootFromJointTransform));
...
void Model::setPrimitiveMode(PrimitiveMode primitiveMode, const render::ScenePointer& scene) {
    if (_primitiveMode != primitiveMode) {
        ...
        for (int i = 0; i < (int)_modelMeshRenderItemIDs.size(); i++) {
            auto itemID = _modelMeshRenderItemIDs[i];
            const auto& shapeState = getShapeState(i);
            bool invalidatePayloadShapeKey = shouldInvalidatePayloadShapeKey(shapeState._meshIndex);
...
void Model::createRenderItemSet() {
    ...
    for (uint32_t shapeID = 0; shapeID < shapes.size(); shapeID++) {
        ...
        _modelMeshRenderItems << std::make_shared<ModelMeshPartPayload>(shared_from_this(), shape.mesh, shape.meshPart, shapeID, transform, _created);
